...
export class InscripcionesService {
  ...
  async create(data: CreateInscripcionDto) {
    ...
    const inscripcionesExistentes =
      await this.inscripcionesRepository.findByEstudianteId(data.estudianteId);

    const yaInscrito = inscripcionesExistentes.some(
      (inscripcion: Inscripcion) =>
        inscripcion.moduloId === data.moduloId,
    );

    if (yaInscrito) {
      throw new NotFoundException(
        'El estudiante ya está inscrito en este módulo',
      );
    }

    const inscripcion = await this.inscripcionesRepository.create({
      ...data,
      numeroInscripcion: this.generarNumeroInscripcion(),
      monto: modulo.costo ?? 0,
    });

    await this.notificacionesService.notificarInscripcion({
      usuarioId: data.estudianteId,
      cursoNombre: modulo.curso.nombre,
  ...
  async eliminarCursoInscripciones(
    idEstudiante: string,
    idCurso: string,
  ) {
    const inscripciones =
      await this.inscripcionesRepository.findByEstudianteInscripciones(
        idEstudiante,
      );

    const inscripcionesCurso = inscripciones.filter(
      (inscripcion) => inscripcion.modulo.curso.id === idCurso,
    );

    if (inscripcionesCurso.length === 0) {
      throw new NotFoundException(
        `No se encontraron inscripciones para el estudiante con id ${idEstudiante} en el curso con id ${idCurso}`,
      );
    }
    ...
    return {
      message: `Se eliminaron ${inscripcionesCurso.length} inscripciones del estudiante con id ${idEstudiante} en el curso con id ${idCurso}`,
    };
  }

  async eliminarModuloInscripciones(
    id: string,
    idCurso: string,
    idModulo: string,
  ) {
    const inscripcion =
      await this.inscripcionesRepository.findByIdWithCurso(id);
    ...
    if (
      inscripcion.modulo.curso.id !== idCurso ||
      inscripcion.modulo.id !== idModulo
    ) {
      throw new NotFoundException(
        `La inscripción con id ${id} no pertenece al curso con id ${idCurso} y módulo con id ${idModulo}`,
      );
    }
    ...
    return {
      message: `Se eliminó la inscripción con id ${id} del estudiante en el curso con id ${idCurso} y módulo con id ${idModulo}`,
    };
  }
  ...
  async eliminarTodasInscripciones(idEstudiante: string) {
    ...
    if (inscripciones.length === 0) {
      throw new NotFoundException(
        `No se encontraron inscripciones para el estudiante con id ${idEstudiante}`,
      );
    }
    ...
    return {
      message: `Se eliminaron todas las inscripciones del estudiante con id ${idEstudiante}`,
    };
  }
  ...
  async findAllPaginated(
    ...
    const [estudiantes, total] = await Promise.all([
      this.inscripcionesRepository.findEstudianteWithInscripciones(
        skip,
        limit,
        search,
      ),
    ...
    const resultado: EstudianteAgrupado[] = estudiantes.map(
      (estudiante) => {
  ...
  async createMultiple(data: CreateInscripcionEstudiantesDto) {
    const [modulos, estudiantes] = await Promise.all([
      Promise.all(
        data.moduloIds.map((id) =>
          this.moduloService.findOne(id),
        ),
      ),
      Promise.all(
        data.estudianteIds.map((id) =>
    ...
    const estudiantesExistentes = estudiantes.filter(
    ...
    const inscripcionesExistentes = await Promise.all(
      estudiantesExistentes.map((estudiante) =>
        this.inscripcionesRepository.findByEstudianteId(
          estudiante.id,
        ),
      ),
    );
    ...
    const paresExistentes = new Set(
      inscripcionesExistentes
        ...
        .map(
          (inscripcion: Inscripcion) =>
            `${inscripcion.estudianteId}-${inscripcion.moduloId}`,
        ),
    );
    ...
    const resultado =
      await this.inscripcionesRepository.createMultiple({
        inscripciones: inscripcionesNuevas,
      });

    await Promise.all(
      inscripcionesNuevas.map((inscripcion) => {
        const modulo = modulosPorId.get(inscripcion.moduloId);

        if (!modulo) return Promise.resolve();

        return this.notificacionesService.notificarInscripcion({
